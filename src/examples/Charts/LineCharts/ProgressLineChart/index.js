...
function ProgressLineChart({ color, icon, title, count, progress, height, chart }) {
  // Notice we now pass chart.datasets instead of chart.data
  const { data, options } = configs(color, chart.labels || [], title, chart.datasets || []);
  console.log("Chart data:", data, "Options:", options);
  return (
    <Card>
